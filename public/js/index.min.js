...
// CONCATENATED MODULE: ./src/Typescript/WeatherDay.ts
...
function dataToDivs(weatherData, howManyReadings) {
    var dataArray = [];
    for (var i = 0; i < howManyReadings; i++) {
        var j = 0;
        var innerDataArray = [];
        ...
        var tempInC = ((parseInt(weatherData['list'][i.toString()]['main']['temp'])) - 273.15).toFixed(0);
        ...
        var iconValue = (weatherData['list'][i.toString()]['weather']['0']['icon']).toString();
        var iconLink = 'https://openweathermap.org/img/wn/' + iconValue + '@2x.png';
...
function appendDataToDiv(dataArray) {
    var li = document.getElementsByClassName("weatherInnerContainer");
    var nI = li.length;
    for (var i = 0; i < nI; i++) {
...
// CONCATENATED MODULE: ./src/Typescript/City.ts
// OOP model with:
// - public name; string for displaying on button
// - public key; string for API usage
function getCity() {
    var city = event.target.innerHTML.toString();
    city;
    return city;
}
function getApi(city) {
    document.getElementById("weatherLocation").innerHTML = city;
    var api = 'https://api.openweathermap.org/data/2.5/forecast?q=' + city + '&appid=ab57b33912fbd3d3015d3f296505d3a8';
    return api;
}

// CONCATENATED MODULE: ./src/Typescript/index.ts
...
function main() {
    var city = getCity();
    var api = getApi(city);
    getWeatherData(api).then(function (weatherData) {
        clearDivs();
        addContainerDivs(howManyReadings);
        var dataArray = dataToDivs(weatherData, howManyReadings);
        appendDataToDiv(dataArray);
    });
}
document.querySelectorAll('.allButtons').forEach(function (item) {
    item.addEventListener('click', function (event) {
        main();
    });
});
function clearDivs() {
    var myNode = document.getElementById("weatherOuterContainer");
    while (myNode.firstChild) {
        myNode.removeChild(myNode.lastChild);
    }
}
function addContainerDivs(howManyReadings) {
    var myNode = document.getElementById("weatherOuterContainer");
    for (var i = 0; i < howManyReadings; i++) {
